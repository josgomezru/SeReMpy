...
# Eq 7
c_theta = 0.5 * (1 - 4 * gamma**2 * np.sin(theta)**2)

rRho = DeltaRho/Rho_bar

d_theta_omega = -0.5 * (1 + np.tan(theta)**2) * (1/np.pi * np.log(omega / omega_r) - 0.5j)

rXip = DeltaXip/Xip_bar

e_theta_omega = -4 * gamma**2 * np.sin(theta)**2 * (1 / np.pi * np.log(omega / omega_r) - 0.5j)

rXis = DeltaXis/Xis_bar
